...
public class Klem implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String s, String[] args) {
        if (sender instanceof Player) {
            ...
            if (args.length == 0) {
                ...
                if (t == p) {
                    p.spigot().sendMessage(ChatMessageType.ACTION_BAR, TextComponent.fromLegacyText(Messages.KLEM_SELV));
                    return true;
                }
                if (t == null) {
                ...
                Bukkit.broadcastMessage(Messages.KLEM.replaceAll("%spiller%", p.getName()).replaceAll("%target%", t.getName()));
